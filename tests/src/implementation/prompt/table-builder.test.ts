import {
  Paragraph,
  Table,
} from "../../../../src/developer-api/prompt/types.js";
...
describe("TableBuilderImpl", () => {
  it("builds a basic table with rows and cells", () => {
    ...
    builder.row((r) =>
      r.cell((c) => c.paragraph((data) => `cell: ${data.foo}`)),
    );
  ...
  it("sets column headers if provided", () => {
    ...
    builder.row((r) =>
      r.cell((c) => c.paragraph`cell1`).cell((c) => c.paragraph`cell2`),
    );
  ...
  it("supports row headers", () => {
    ...
    builder.row((r) =>
      r
        .header((data) => `row header: ${data.foo}`)
        .cell((c) => c.paragraph`cell`),
    );
    const table = builder.build({ foo: "rowfoo" });
    expect(table.rows[0].rowHeader).toBeDefined();
    expect(table.rows[0].rowHeader!.contents.content).toBe(
      "row header: rowfoo",
    );
  });
  ...
  it("parameterizes data for dynamic content", () => {
    ...
    builder.row((r) =>
      r
        .header((data) => `row: ${data.foo}`)
        .cell((c) => c.paragraph((data) => data.bar)),
    );
