...
describe("TableCellBuilderImpl", () => {
  ...
  it("builds a cell with a list", () => {
    ...
    if (list && list.type === "list") {
      ...
      expect(list.items[1].content).toEqual({
        type: "paragraph",
        content: "Another: X",
      });
    }
  ...
  it("builds a cell with a nested table", () => {
    ...
    builder.table((t) =>
      t.row((r) => r.cell((c) => c.paragraph((data) => `Cell: ${data.val}`))),
    );
    ...
    if (table && table.type === "table") {
      ...
      expect(table.rows[0].cells[0].contents[0]).toEqual({
        type: "paragraph",
        content: "Cell: Y",
      });
    }
  ...
  it("builds a cell with mixed content", () => {
    ...
    builder.table((t) =>
      t.row((r) => r.cell((c) => c.paragraph((data) => `T: ${data.foo}`))),
    );
