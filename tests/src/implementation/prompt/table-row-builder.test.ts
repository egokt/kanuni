...
describe('TableRowBuilderImpl build method', () => {
  ...
  it('builds a row with a single cell', () => {
    const row: TableRowBuilderImpl<typeof data> = new TableRowBuilderImpl<typeof data>();
    row.cell(c => c.paragraph((d: typeof data) => `Cell: ${d.title}`));
    const result = row.build(data);
    expect(result.cells).toHaveLength(1);
    expect(result.cells[0].contents[0]).toEqual({ type: 'paragraph', content: 'Cell: Test Title' });
    expect(result.rowHeader).toBeUndefined();
  });

  it('builds a row with multiple cells', () => {
    const row: TableRowBuilderImpl<typeof data> = new TableRowBuilderImpl<typeof data>();
    row.cell(c => c.paragraph(d => `Cell 1: ${d.title}`))
      .cell(c => c.paragraph(d => `Cell 2: ${d.value}`));
    const result = row.build(data);
    expect(result.cells).toHaveLength(2);
    expect(result.cells[0].contents[0]).toEqual({ type: 'paragraph', content: 'Cell 1: Test Title' });
    expect(result.cells[1].contents[0]).toEqual({ type: 'paragraph', content: 'Cell 2: 42' });
  });
  ...
  it('builds a row with a header (function overload)', () => {
    ...
    expect(result.rowHeader?.contents).toEqual({ type: 'paragraph', content: 'Header: Test Title' });
  ...
  it('cell content is built with correct data', () => {
    ...
    expect(result.cells[0].contents[0]).toEqual({ type: 'paragraph', content: 'Cell: 999' });
  });
  ...
  it('cell order is preserved', () => {
    const row = new TableRowBuilderImpl<typeof data>();
    row.cell((c: any) => c.paragraph((d: typeof data) => `Cell1: ${d.title}`))
    ...
    expect(result.cells[0].contents[0]).toEqual({ type: 'paragraph', content: 'Cell1: B' });
    expect(result.cells[1].contents[0]).toEqual({ type: 'paragraph', content: 'Cell2: 7' });
