...
describe('QueryBuilderImpl', () => {
  it('returns correct prompt contents when only prompt is set', () => {
    ...
    expect(result).toEqual({
      prompt: {
        type: 'prompt',
        contents: [ expect.objectContaining({ type: 'paragraph', content: 'Prompt: bar' }) ],
      },
    });
  });
  ...
  it('uses data parameter correctly in prompt', () => {
    ...
    expect(result.prompt.contents[0]).toEqual(expect.objectContaining({ type: 'paragraph', content: 'Hello, Alice' }));
  });
  ...
  it('handles memory builder returning undefined', () => {
    ...
    builder.prompt(p => p.paragraph`Prompt: ${'foo'}`);
    const result = builder.build({ foo: 'bar' });
    expect(result.prompt.contents[0]).toEqual(expect.objectContaining({ type: 'paragraph', content: 'Prompt: bar' }));
  });
  ...
  it('is chainable and builds correctly', () => {
    ...
    const result = builder.build({ foo: 'bar' });
    expect(result.prompt.contents[0]).toEqual(expect.objectContaining({ type: 'paragraph', content: 'Chainable: bar' }));
  });
});
