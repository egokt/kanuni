...
describe('ListBuilder', () => {
  ...
  it('builds a nested list with .list()', () => {
    ...
    expect((((result.items[1].content as List).items[0].content as Paragraph).content)).toBe('B');
  });
  ...
  it('builds a deeply nested list', () => {
    ...
    expect((result.items[0].content as List).items[0].content.type).toBe('list');
    expect((((result.items[0].content as List).items[0].content as List).items[0].content as Paragraph).content).toBe('z');
  });
  ...
  it('builds a list with mixed .item(), .list(), and .items()', () => {
    ...
    expect(result.items[1].content.type).toBe('list');
    expect(((result.items[1].content as List).items[0].content as Paragraph).content).toBe('sub');
  ...
  it('passes data through all builder functions', () => {
    ...
    expect(((result.items[1].content as List).items[0].content as Paragraph).content).toBe('bar!');
  });
});
